...
**Version 0.4.1-pilot is ready for a next-machine evaluation, not a company-wide performance guarantee.** See the [optimization report](docs/optimization-results.md) for the capped follow-up experiment; the earlier token overhead remains in [the historical report](docs/token-benchmark-results.md). No lengthy benchmark is required to install or try this package.

For explanation tasks, verify that Copilot requests `-Compact -IncludeContext` and treats the returned numbered excerpts as current source reads. It should read only missing ranges, keep default limits initially, and stop when the requested example is established. Candidate-only questions need no context. Do not require the same tool sequence for semantic navigation, unsaved buffers or a file already known by name.
